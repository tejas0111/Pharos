...
test("mcp-server/index.js exists", () => {
  assert(existsSync("index.js"), "index.js not found");
});
...
test("index.js has valid syntax", () => {
  execSync("node --check index.js 2>/dev/null", { stdio: "pipe" });
});

// 4. Viem import resolves (runtime dependency check)
test("viem import resolves at runtime", () => {
  execSync('node -e "import(\'viem\')" 2>/dev/null', { stdio: "pipe" });
});



// 6. All 11 tools are registered by name
...
// 8. TOOL_SCHEMAS has all 11 tools
...
// 9. SUBLINK map has all 11 tools
...
// 10. withSubskill function wired to tools
...
// 11. Structured error hints present
...
// 9. Pharos-specific security checks present
...
// 10. pharos_diagnose tool exists
