...
3. List the methods, events, and revert paths that must be exposed.
...
5. Normalize naming so frontend and backend tooling can use the same surface.
6. Show the ABI/interface plan and ask if the shape is correct.
7. Generate the interface or binding skeleton after confirmation.
## Output
